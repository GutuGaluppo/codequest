...
export async function generateTutorial(
	...
	if (preferredModel === "claude" && userKeys.anthropic) {
		try {
			return await claudeService.generate(topic, userKeys.anthropic);
		} catch (error) {
			console.warn(
	...
	if (preferredModel === "openai" && userKeys.openai) {
		try {
			return await openaiService.generate(topic, userKeys.openai);
		} catch (error) {
			console.warn(
	...
	return geminiService.generate(topic, import.meta.env.VITE_GEMINI_API_KEY);
}
